...
const WhatsAppWidget = () => {
  ...
  const phoneNumber = "+918287933634" // Your number from the image
  const message = "Hi! I'm interested in your CBG plant projects."
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {isOpen && (
        <div className="mb-4 bg-white rounded-lg shadow-xl px-4 py-2 w-64 animate-fadeIn border border-green-200">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h4 className="font-bold text-gray-800">KEC Support</h4>
            </div>
          </div>
          <p className="text-sm text-gray-600 mb-2">
            Hi! How can we help you with your sustainable energy needs?
          </p>
          <button
            onClick={openWhatsApp}
            className="w-full bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded-lg text-sm font-bold transition-colors"
          >
            Start Chat
          </button>
        </div>
      ...
      {/* WhatsApp button with logo overlay */}
      <div className="relative">
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="bg-green-500 hover:bg-green-600 text-white rounded-full p-4 shadow-lg transition-all duration-300 hover:scale-110 relative overflow-hidden"
        >
