...
const FONTS_URL = "https://fonts.googleapis.com/css2?family=Syne:wght@400;500;600;700;800&family=DM+Sans:wght@300;400;500;600&display=swap";
...
function generateInsights(answers) {
    ...
    return { cbgTPD, cbgAnnual, cbgRevenue, digestateRevenue, totalRevenue, opex, ebitda,
...
function MeshBackground() {
    return (
        <div style={{ position:'absolute', inset:0, overflow:'hidden', pointerEvents:'none' }}>
            <div style={{ position:'absolute', inset:0, background:'radial-gradient(ellipse 80% 60% at 50% -10%, rgba(16,185,129,0.18) 0%, transparent 60%)' }} />
            <div style={{ position:'absolute', inset:0, background:'radial-gradient(ellipse 60% 50% at 80% 80%, rgba(245,158,11,0.10) 0%, transparent 55%)' }} />
            {[
                { size:320, x:'10%', y:'20%', delay:0, dur:18, color:'rgba(16,185,129,0.07)' },
                { size:200, x:'75%', y:'10%', delay:3, dur:14, color:'rgba(245,158,11,0.08)' },
                { size:260, x:'60%', y:'65%', delay:6, dur:22, color:'rgba(16,185,129,0.06)' },
            ].map((o,i) => (
                <motion.div key={i}
                    style={{ position:'absolute', width:o.size, height:o.size, left:o.x, top:o.y,
                             borderRadius:'50%', background:o.color, filter:'blur(60px)' }}
            ...
            {Array.from({length:20},(_,i)=>({ size: Math.random()*3+1, x: Math.random()*100, delay: Math.random()*8, dur: Math.random()*10+8, op: Math.random()*0.15+0.05 })).map((p,i) => (
                <motion.div key={i} style={{ position:'absolute', width:p.size, height:p.size, left:`${p.x}%`, bottom:'-4px', borderRadius:'50%', background:'white', opacity:p.op }}
                    animate={{ y:[0,-600], opacity:[p.op,0] }}
                    ...
                    <div key={s.id} style={{ display:'flex', alignItems:'center', flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:5, flex:1 }}>
                            <div style={{ position:'relative' }}>
                                {active && (
                                    <motion.div style={{ position:'absolute', inset:-6, borderRadius:'50%', border:'2px solid rgba(16,185,129,0.35)' }}
                                        animate={{ scale:[1,1.4,1], opacity:[0.5,0,0.5] }}
                                        transition={{ duration:2.2, repeat:Infinity }} />
                                )}
                                <motion.div animate={active ? { scale:1.2 } : { scale:1 }}
                                    transition={{ type:'spring', stiffness:300 }}
                                    style={{ width:40, height:40, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center',
                                        background: done ? 'linear-gradient(135deg,#10b981,#059669)' : active ? 'linear-gradient(135deg,#f59e0b,#d97706)' : 'rgba(255,255,255,0.05)',
                                    ...
                                    {done ? <CheckCircle2 size={16} color="white" /> : <Icon size={14} color={active?'white':'rgba(255,255,255,0.35)'} />}
                                </motion.div>
                            </div>
                            <span style={{ fontSize:9, fontFamily:'DM Sans,sans-serif', fontWeight:600, letterSpacing:'0.04em',
                                color: active ? '#f59e0b' : done ? '#10b981' : 'rgba(255,255,255,0.25)',
                                whiteSpace:'nowrap' }}>
                                {s.title}
                            </span>
                        </div>
                        {i < STEPS.length - 1 && (
                            <div style={{ flex:1, height:1, margin:'0 6px', background:'rgba(255,255,255,0.07)', maxWidth:80, borderRadius:4, overflow:'hidden' }}>
...
function OptionCard({ label, selected, onClick, multi, accent }) {
    ...
    const emGlow  = accent === 'amber' ? 'rgba(245,158,11,0.22)' : 'rgba(16,185,129,0.22)';
    const emBg    = accent === 'amber' ? 'rgba(245,158,11,0.07)' : 'rgba(16,185,129,0.07)';

    return (
        <motion.div onMouseMove={handleMove} onMouseLeave={handleLeave} onClick={onClick}
            style={{ perspective:600, rotateX, rotateY, cursor:'pointer', borderRadius:13,
                border: selected ? `1.5px solid ${emColor}` : '1.5px solid rgba(255,255,255,0.07)',
                background: selected ? emBg : 'rgba(255,255,255,0.02)', padding:'11px 15px',
            ...
            whileHover={{ scale:1.012, background: selected ? emBg : 'rgba(255,255,255,0.04)' }}
            whileTap={{ scale:0.985 }}>
            <div style={{ width:19, height:19, borderRadius: multi ? 5 : '50%', flexShrink:0,
                border: selected ? `2px solid ${emColor}` : '2px solid rgba(255,255,255,0.18)',
            ...
            <span style={{ fontFamily:'DM Sans,sans-serif', fontSize:13.5, fontWeight:500,
                color: selected ? 'rgba(255,255,255,0.92)' : 'rgba(255,255,255,0.52)', transition:'color 0.18s', flex:1 }}>
                {label}
            </span>
            {selected && (
    ...
    return (
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <div style={{ position:'relative', width:84, height:84 }}>
                <motion.div style={{ position:'absolute', inset:0, borderRadius:'50%', border:'2px solid transparent',
                    borderTopColor:'#10b981', boxShadow:'0 0 24px rgba(16,185,129,0.5)' }}
                    animate={{ rotate:360 }} transition={{ duration:0.9, repeat:Infinity, ease:'linear' }} />
                <motion.div style={{ position:'absolute', inset:12, borderRadius:'50%', border:'2px solid transparent',
                    borderTopColor:'#f59e0b', borderBottomColor:'rgba(245,158,11,0.3)' }}
            ...
            <div style={{ textAlign:'center' }}>
                <div style={{ fontFamily:'Syne,sans-serif', fontSize:17, fontWeight:700, color:'white', marginBottom:10 }}>
                    BioPulse AI Processing
                </div>
                <AnimatePresence mode="wait">
                    <motion.p key={phase} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}}
                        transition={{duration:0.25}}
                        style={{ fontFamily:'DM Sans,sans-serif', fontSize:13, color:'rgba(255,255,255,0.4)', margin:0 }}>
                        {phases[phase]}
                    </motion.p>
                </AnimatePresence>
            </div>
                ...
                {phases.map((_,i) => (
                    <motion.div key={i} animate={{ scale: i===phase ? 1.4 : 1 }}
                        style={{ width:7, height:7, borderRadius:'50%',
                            background: i<=phase ? '#10b981' : 'rgba(255,255,255,0.12)',
...
function ResultsScreen({ answers, onRestart }) {
    ...
    const urgencyConfig = {
        HIGH:   { color:'#ef4444', glow:'rgba(239,68,68,0.3)',  label:'HIGH URGENCY',   bg:'rgba(239,68,68,0.07)',   border:'rgba(239,68,68,0.22)' },
        MEDIUM: { color:'#f59e0b', glow:'rgba(245,158,11,0.3)', label:'MEDIUM URGENCY', bg:'rgba(245,158,11,0.07)',  border:'rgba(245,158,11,0.22)' },
        LOW:    { color:'#3b82f6', glow:'rgba(59,130,246,0.3)', label:'EXPLORING',      bg:'rgba(59,130,246,0.07)',  border:'rgba(59,130,246,0.22)' },
    };
    ...
    const checks = [
        ...
        { label:'CBG Park Match', val:r.parkSuitable?'High Suitability':'Standard Plant', desc:r.parkSuitable?'Park model aligns':'Standalone recommended', icon:Award },
    ];

    return (
        <motion.div initial={{opacity:0}} animate={{opacity:1}} style={{ display:'flex', flexDirection:'column', gap:18 }}>
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.1}}
                style={{ position:'relative', borderRadius:20, overflow:'hidden', padding:'26px 22px', textAlign:'center',
                ...
                <div style={{ position:'absolute', top:-40, right:-40, width:150, height:150,
                    background:'radial-gradient(circle,rgba(16,185,129,0.18),transparent 70%)', borderRadius:'50%' }} />
                <div style={{ position:'absolute', bottom:-30, left:-30, width:110, height:110,
                    background:'radial-gradient(circle,rgba(245,158,11,0.12),transparent 70%)', borderRadius:'50%' }} />
                <div style={{ position:'relative' }}>
                    ...
                    <h2 style={{ fontFamily:'Syne,sans-serif', fontSize:21, fontWeight:800, color:'white', marginBottom:8, lineHeight:1.3 }}>
                        You're well-positioned for a<br/>
                        <span style={{ color:'#34d399' }}>scalable CBG opportunity</span>
                    </h2>
                    <p style={{ fontFamily:'DM Sans,sans-serif', color:'rgba(255,255,255,0.5)', fontSize:13, lineHeight:1.6, maxWidth:420, margin:'0 auto' }}>
            ...
            <motion.div initial={{opacity:0,x:-16}} animate={{opacity:1,x:0}} transition={{delay:0.22}}
                ...
                <div>
                    <div style={{ fontFamily:'Syne,sans-serif', fontSize:10, fontWeight:700, letterSpacing:'0.1em', color:uc.color, marginBottom:4 }}>
                        MARKET URGENCY: {uc.label}
                    </div>
                    <p style={{ fontFamily:'DM Sans,sans-serif', fontSize:12, color:'rgba(255,255,255,0.5)', lineHeight:1.55, margin:0 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                {kpis.map((k,i) => (
                    <motion.div key={k.label} initial={{opacity:0,y:20,scale:0.92}} animate={{opacity:1,y:0,scale:1}}
                        ...
                        style={{ padding:'16px 14px', borderRadius:16, background:'rgba(255,255,255,0.03)',
                        ...
                        <div style={{ position:'absolute', top:-18, right:-18, width:64, height:64,
                            background:`radial-gradient(circle,${k.color}25,transparent 70%)`, borderRadius:'50%' }} />
                        <k.icon size={17} color={k.color} style={{ marginBottom:9 }} />
                        <div style={{ fontFamily:'Syne,sans-serif', fontSize:19, fontWeight:800, color:'white', marginBottom:2 }}>{k.value}</div>
                        <div style={{ fontFamily:'DM Sans,sans-serif', fontSize:11, fontWeight:600, color:'rgba(255,255,255,0.45)', letterSpacing:'0.03em' }}>{k.label}</div>
                        <div style={{ fontFamily:'DM Sans,sans-serif', fontSize:10, color:'rgba(255,255,255,0.28)', marginTop:2 }}>{k.sub}</div>
                    </motion.div>
                ))}
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:9 }}>
                {checks.map((c,i) => (
                    <motion.div key={c.label} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:0.52+i*0.06}}
                        style={{ padding:'13px', borderRadius:13, background:'rgba(16,185,129,0.04)', border:'1px solid rgba(16,185,129,0.13)' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:5 }}>
                            <CheckCircle2 size={11} color="#10b981" />
                            <span style={{ fontFamily:'Syne,sans-serif', fontSize:9, fontWeight:700, letterSpacing:'0.08em',
                                color:'rgba(255,255,255,0.3)', textTransform:'uppercase' }}>{c.label}</span>
                        </div>
                        <div style={{ fontFamily:'DM Sans,sans-serif', fontSize:12.5, fontWeight:600, color:'#34d399', marginBottom:3 }}>{c.val}</div>
            ...
            <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.78}}
                style={{ padding:'18px', borderRadius:16, background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.06)' }}>
                <div style={{ fontFamily:'Syne,sans-serif', fontSize:12, fontWeight:700, color:'rgba(255,255,255,0.6)',
                    marginBottom:14, display:'flex', alignItems:'center', gap:7, letterSpacing:'0.06em' }}>
                ...
                {[
                    ...
                    <div key={rb.label} style={{ marginBottom:13 }}>
                        <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5 }}>
                            <span style={{ fontFamily:'DM Sans,sans-serif', fontSize:12, color:'rgba(255,255,255,0.5)' }}>{rb.label}</span>
                            <span style={{ fontFamily:'Syne,sans-serif', fontSize:12, fontWeight:700, color:'white' }}>{fmt(rb.value)}</span>
                        </div>
                ...
                <div style={{ paddingTop:12, borderTop:'1px solid rgba(255,255,255,0.05)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <span style={{ fontFamily:'DM Sans,sans-serif', fontSize:13, fontWeight:500, color:'rgba(255,255,255,0.45)' }}>Total Annual Revenue</span>
                    <span style={{ fontFamily:'Syne,sans-serif', fontSize:20, fontWeight:800, color:'#10b981', textShadow:'0 0 20px rgba(16,185,129,0.45)' }}>{fmt(r.totalRevenue)}</span>
                </div>
            </motion.div>

            <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.95}}
                style={{ display:'flex', gap:9, padding:'11px 13px', borderRadius:11, background:'rgba(245,158,11,0.05)', border:'1px solid rgba(245,158,11,0.15)' }}>
                <Sparkles size={13} color="#f59e0b" style={{flexShrink:0,marginTop:2}} />
                <p style={{ fontFamily:'DM Sans,sans-serif', fontSize:11, color:'rgba(255,255,255,0.38)', lineHeight:1.6, margin:0 }}>
            ...
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:1.05}}
                style={{ borderRadius:20, overflow:'hidden', padding:'26px 22px', textAlign:'center', position:'relative',
                ...
                <h3 style={{ fontFamily:'Syne,sans-serif', fontSize:17, fontWeight:800, color:'white', marginBottom:6 }}>Ready to build your CBG plant?</h3>
                <p style={{ fontFamily:'DM Sans,sans-serif', color:'rgba(255,255,255,0.5)', fontSize:12, marginBottom:22 }}>Get a detailed DPR, approvals assistance, and end-to-end EPC support from KEC.</p>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:10, marginBottom:14 }}>
                    <div style={{ height:1, width:36, background:'rgba(255,255,255,0.12)' }} />
                    <span style={{ fontFamily:'Syne,sans-serif', fontSize:9, color:'rgba(52,211,153,0.65)', letterSpacing:'0.12em', fontWeight:700 }}>TALK TO OUR EXPERT</span>
                    <div style={{ height:1, width:36, background:'rgba(255,255,255,0.12)' }} />
                </div>
                <div style={{ display:'flex', flexDirection:'column', gap:9, maxWidth:320, margin:'0 auto 14px' }}>
                    <motion.a href="tel:+918527626868" whileHover={{scale:1.02,boxShadow:'0 8px 30px rgba(0,0,0,0.4)'}} whileTap={{scale:0.98}}
                        style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:12,
                            background:'white', borderRadius:13, padding:'13px 18px', textDecoration:'none',
                            boxShadow:'0 6px 24px rgba(0,0,0,0.25)' }}>
                        <span style={{fontSize:17}}>📞</span>
                        <div style={{textAlign:'left'}}>
                            <div style={{fontFamily:'DM Sans,sans-serif',fontSize:10,color:'#9ca3af',fontWeight:500}}>Call us now</div>
                            <div style={{fontFamily:'Syne,sans-serif',fontSize:14,fontWeight:800,color:'#111'}}>+91 85276 26868</div>
                        </div>
                    </motion.a>
                    <motion.a href="https://wa.me/918287933634?text=Hello%20KEC%20Team,%20I%20completed%20the%20BioPulse%20AI%20analysis%20and%20want%20a%20detailed%20consultation."
                        target="_blank" rel="noopener noreferrer"
                        whileHover={{scale:1.02,boxShadow:'0 8px 30px rgba(37,211,102,0.3)'}} whileTap={{scale:0.98}}
                        style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:12,
                            background:'#25D366', borderRadius:13, padding:'13px 18px', textDecoration:'none',
                            boxShadow:'0 6px 24px rgba(37,211,102,0.2)' }}>
                        <svg viewBox="0 0 32 32" fill="white" width="18" height="18"><path d="M16 .4C7.4.4.4 7.4.4 16c0 2.7.7 5.3 2 7.6L.4 31.6l8.2-2c2.2 1.2 4.7 1.8 7.4 1.8 8.6 0 15.6-7 15.6-15.6C31.6 7.4 24.6.4 16 .4zm0 28.6c-2.4 0-4.7-.6-6.7-1.8l-.5-.3-4.9 1.2 1.3-4.7-.3-.5C3.7 20.8 3 18.5 3 16 3 8.8 8.8 3 16 3s13 5.8 13 13-5.8 13-13 13zm7.1-9.7c-.4-.2-2.3-1.1-2.6-1.2-.4-.1-.6-.2-.9.2-.3.4-1 1.2-1.2 1.5-.2.2-.5.3-.9.1-.4-.2-1.7-.6-3.2-2-1.2-1-2-2.3-2.2-2.7-.2-.4 0-.6.2-.8.2-.2.4-.5.6-.7.2-.2.3-.4.4-.6.1-.2 0-.5-.1-.7-.1-.2-.9-2.1-1.2-2.9-.3-.7-.6-.6-.9-.6h-.7c-.3 0-.7.1-1 .4-.4.4-1.3 1.3-1.3 3.1s1.3 3.6 1.5 3.9c.2.2 2.6 4 6.4 5.6.9.4 1.6.6 2.1.8.9.3 1.7.2 2.4.1.7-.1 2.3-.9 2.6-1.8.3-.9.3-1.6.2-1.8-.1-.2-.4-.3-.8-.5z"/></svg>
                        <div style={{textAlign:'left'}}>
                            <div style={{fontFamily:'DM Sans,sans-serif',fontSize:10,color:'rgba(255,255,255,0.65)',fontWeight:500}}>Chat instantly</div>
                            <div style={{fontFamily:'Syne,sans-serif',fontSize:14,fontWeight:800,color:'white'}}>WhatsApp Us</div>
                        </div>
                    </motion.a>
                </div>
                <motion.button onClick={onRestart} whileHover={{scale:1.03,background:'rgba(255,255,255,0.1)'}} whileTap={{scale:0.97}}
                    style={{ display:'inline-flex', alignItems:'center', gap:7, padding:'9px 18px', borderRadius:10,
                        background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)',
                        color:'rgba(255,255,255,0.45)', fontFamily:'DM Sans,sans-serif', fontSize:12, fontWeight:600, cursor:'pointer' }}>
...
export default function KecBioPulseAI() {
    ...
    const handleChange = (id, value) => { setAnswers(p=>({...p,[id]:value})); setErrors(p=>({...p,[id]:false})); };
    const validate = () => {
        const e = {};
    ...
    const handleRestart = () => { setStep(1); setAnswers({}); setErrors({}); setPhase('form'); cardRef.current?.scrollIntoView({behavior:'smooth',block:'start'}); };

    return (
        <div style={{ minHeight:'100vh', background:'#070d09' }}>

            {/* ── HERO ── */}
            <div style={{ position:'relative', background:'linear-gradient(180deg,#0a1a0e 0%,#070d09 100%)', overflow:'hidden' }}>
                <MeshBackground />
                <div style={{ position:'relative', maxWidth:900, margin:'0 auto', padding:'60px 20px 68px', textAlign:'center' }}>

                    {/* ── ROW 1: Logo pill — standalone row ── */}
                    <motion.div initial={{opacity:0,y:-18}} animate={{opacity:1,y:0}}
                        ...
                        <div style={{ display:'inline-flex', alignItems:'center', gap:10,
                            ...
                            />
                            {/* Fallback leaf icon */}
                            <div style={{ display:'none', width:30, height:30, borderRadius:8,
                                background:'rgba(16,185,129,0.18)', alignItems:'center', justifyContent:'center' }}>
                                <Leaf size={15} color="#10b981" />
                            </div>

                            {/* ── FIX: KEC on line 1, Biofuel on line 2 ── */}
                    ...
                    {/* Title */}
                    <motion.h1 initial={{opacity:0,y:22}} animate={{opacity:1,y:0}} transition={{delay:0.14}}
                        style={{ fontFamily:'Syne,sans-serif', fontSize:'clamp(36px,8vw,70px)', fontWeight:900,
                            color:'white', lineHeight:1.05, marginBottom:10, letterSpacing:'-0.025em' }}>
                        KEC{' '}
                        <span style={{ background:'linear-gradient(135deg,#34d399 0%,#10b981 40%,#6ee7b7 100%)',
                    ...
                    <motion.p initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.2}}
                        style={{ fontFamily:'Syne,sans-serif', fontSize:15, fontWeight:600, color:'#f59e0b', letterSpacing:'0.05em', marginBottom:14 }}>
                        AI Feasibility Engine
                    </motion.p>

                    <motion.p initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:0.26}}
                        style={{ fontFamily:'DM Sans,sans-serif', fontSize:14, color:'rgba(255,255,255,0.4)', maxWidth:440, margin:'0 auto 36px', lineHeight:1.75 }}>
                    ...
                    <motion.div initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{delay:0.33}}
                        ...
                        {[['100+','Plants Built'],['₹500Cr+','Investments'],['30%','NABARD Subsidy'],['95%','Plant Uptime']].map(([n,l])=>(
                            <div key={l} style={{ textAlign:'center' }}>
                                <div style={{ fontFamily:'Syne,sans-serif', fontSize:24, fontWeight:800, color:'#f59e0b', textShadow:'0 0 18px rgba(245,158,11,0.4)' }}>{n}</div>
                                <div style={{ fontFamily:'DM Sans,sans-serif', fontSize:10, color:'rgba(255,255,255,0.3)', fontWeight:500, letterSpacing:'0.04em', marginTop:2 }}>{l}</div>
                            </div>
                        ))}
                    </motion.div>
                </div>
            ...
            <div style={{ maxWidth:720, margin:'0 auto', padding:'0 14px 72px' }} ref={cardRef}>
                <motion.div initial={{opacity:0,y:28}} animate={{opacity:1,y:0}} transition={{delay:0.28,duration:0.5}}
                    style={{ background:'rgba(255,255,255,0.025)', borderRadius:26, overflow:'hidden',
                        border:'1px solid rgba(255,255,255,0.065)',
                        boxShadow:'0 40px 90px rgba(0,0,0,0.55), 0 0 0 1px rgba(255,255,255,0.035)',
                        backdropFilter:'blur(16px)' }}>

                    {phase==='form' && (
                        <div style={{ padding:'18px 24px 0', borderBottom:'1px solid rgba(255,255,255,0.055)' }}>
                            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:2 }}>
                                <div>
                                    <div style={{ fontFamily:'Syne,sans-serif', fontSize:14, fontWeight:700, color:'rgba(255,255,255,0.85)' }}>Investor Profile Engine</div>
                                    <div style={{ fontFamily:'DM Sans,sans-serif', fontSize:11, color:'rgba(255,255,255,0.28)', marginTop:2 }}>All fields help BioPulse AI generate a more accurate analysis</div>
                                </div>
                                <div style={{ padding:'4px 12px', borderRadius:99, flexShrink:0, marginLeft:10,
                                    background:`rgba(${accent==='amber'?'245,158,11':'16,185,129'},0.1)`,
                                    border:`1px solid rgba(${accent==='amber'?'245,158,11':'16,185,129'},0.22)`,
                                    color:emColor, fontFamily:'Syne,sans-serif', fontSize:11, fontWeight:700 }}>
                                    {step} / {STEPS.length}
                                </div>
                            </div>
                                ...
                                <motion.div key={step} initial={{opacity:0,x:36}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-36}}
                                    ...
                                    <div style={{ marginBottom:22 }}>
                                        <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}
                                            style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'4px 11px', borderRadius:99, marginBottom:9,
                                                background:`rgba(${accent==='amber'?'245,158,11':'16,185,129'},0.09)`,
                                                border:`1px solid rgba(${accent==='amber'?'245,158,11':'16,185,129'},0.18)`,
                                                color:emColor, fontSize:9, fontFamily:'Syne,sans-serif', fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase' }}>
                                            <currentStep.icon size={10} /> {currentStep.title}
                                        </motion.div>
                                        <h3 style={{ fontFamily:'Syne,sans-serif', fontSize:21, fontWeight:800, color:'white', margin:0 }}>{currentStep.subtitle}</h3>
                                    </div>
                                    ...
                                    <div style={{ display:'flex', flexDirection:'column', gap:26 }}>
                                        {currentStep.questions.map((q,qi) => (
                                            <motion.div key={q.id} initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{delay:qi*0.07}}>
                                                <label style={{ display:'block', fontFamily:'DM Sans,sans-serif', fontSize:13.5, fontWeight:600,
                                                    color:'rgba(255,255,255,0.7)', marginBottom:9 }}>
                                                    <span style={{marginRight:5}}>{q.icon}</span>{q.label}
                                                    <span style={{color:'#f59e0b',marginLeft:3}}>*</span>
                                                </label>
                                                {q.hint && (
                                                    <div style={{ display:'flex', gap:7, padding:'7px 11px', borderRadius:9,
                                                        background:'rgba(16,185,129,0.06)', border:'1px solid rgba(16,185,129,0.18)', marginBottom:9 }}>
                                                        <Zap size={11} color="#10b981" style={{flexShrink:0,marginTop:1}} />
                                                        <p style={{ fontFamily:'DM Sans,sans-serif', fontSize:11, color:'rgba(255,255,255,0.4)', margin:0, lineHeight:1.5 }}>{q.hint}</p>
                                                    </div>
                                                )}
                                                ...
                                                {errors[q.id] && (
                                                    <motion.p initial={{opacity:0}} animate={{opacity:1}}
                                                        style={{ display:'flex', alignItems:'center', gap:5, marginTop:7,
                                                            fontFamily:'DM Sans,sans-serif', fontSize:11.5, color:'#f87171', fontWeight:500 }}>
                                    ...
                                    <div style={{ display:'flex', gap:10, marginTop:32, paddingTop:22, borderTop:'1px solid rgba(255,255,255,0.055)' }}>
                                        {step>1 && (
                                            <motion.button whileHover={{scale:1.02}} whileTap={{scale:0.97}} onClick={()=>setStep(s=>s-1)}
                                                style={{ display:'flex', alignItems:'center', gap:7, padding:'13px 18px', borderRadius:13,
                                                    border:'1px solid rgba(255,255,255,0.1)', background:'rgba(255,255,255,0.03)',
                                                    color:'rgba(255,255,255,0.45)', fontFamily:'DM Sans,sans-serif', fontSize:13, fontWeight:600, cursor:'pointer' }}>
                                        ...
                                        <motion.button whileHover={{scale:1.015, boxShadow:`0 0 28px ${emGlow}`}}
                                            ...
                                            style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:9,
                                                ...
                                                boxShadow: accent==='amber' ? '0 4px 20px rgba(217,119,6,0.3)' : '0 4px 20px rgba(5,150,105,0.3)',
                ...
                <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.65}}
                    ...
                    {['100+ Plants Built','SATAT Registered','NABARD Empanelled','EPC + O&M Guarantee'].map(t=>(
                        <div key={t} style={{ display:'flex', alignItems:'center', gap:6 }}>
                            ...
                            <span style={{ fontFamily:'DM Sans,sans-serif', fontSize:11, fontWeight:500, color:'rgba(255,255,255,0.28)' }}>{t}</span>
                        </div>
                    ))}
                </motion.div>
            </div>
